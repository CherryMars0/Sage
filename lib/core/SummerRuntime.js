const SummerRuntime = {
    ...
    exec: (exec) => {
        if (typeof exec === 'function') {
            ...
            console.error(exec.name + 'is not a mod !');
        };
    },


    render(componment) {
        let script = SummerRuntime.regGeneratorScript(componment);
        let css = "<style type='text/css' scoped>" + SummerRuntime.regGeneratorCss(componment) + "</style>";
        let html = SummerRuntime.regGeneratorHtml(componment);
        let outSide = SummerRuntime.regGeneratorOutside(componment);
        console.log(outSide);
